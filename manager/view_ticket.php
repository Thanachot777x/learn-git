...
if (!$ticket_id) {
    header("Location: " . BASE_URL . "/manager/assign_tickets.php");
    exit();
}

// ดึงรายละเอียด Ticket (Manager ดูได้ทุก ticket)
$stmt = $pdo->prepare("
    SELECT t.*, 
...
.btn-manage:hover { background: #1d4ed8; border-color: #1d4ed8; color: #fff; }
...
<div class="view-header">
    <div class="d-flex align-items-center gap-3">
        <a href="<?= BASE_URL ?>/manager/assign_tickets.php" class="btn-back">
    ...
    <div class="d-flex align-items-center gap-2">
        ...
        <a href="<?= BASE_URL ?>/manager/assign_tickets.php" class="btn-manage">
...
<div class="row g-4">
    ...
    <!-- ฝั่งขวา: ข้อมูลสถานที่, อุปกรณ์, ผู้แจ้ง & ช่างผู้รับผิดชอบ -->
    <div class="col-lg-4">
        ...
        <!-- ผู้แจ้ง -->
        <div class="ticket-card mb-4">
            <h6 class="fw-bold text-dark border-bottom pb-2 mb-3"><i class="bi bi-person-badge text-primary me-2"></i>ผู้แจ้ง</h6>
            <div class="d-flex align-items-center gap-3">
                <div class="rounded-circle bg-primary text-white d-flex align-items-center justify-content-center fw-bold" style="width:44px; height:44px; font-size:18px;">
                    <?= mb_substr($ticket['reporter_name'], 0, 1, 'UTF-8') ?>
                </div>
                <div>
                    <div class="fw-bold text-dark"><?= htmlspecialchars($ticket['reporter_name']) ?></div>
                    <div class="text-muted small"><?= htmlspecialchars($ticket['reporter_dept'] ?: '-') ?></div>
                    <?php if (!empty($ticket['reporter_email'])): ?>
                        <div class="text-primary small mt-1"><i class="bi bi-envelope me-1"></i><?= htmlspecialchars($ticket['reporter_email']) ?></div>
                    <?php endif; ?>
                </div>
            </div>
        </div>
